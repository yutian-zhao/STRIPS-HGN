...
if __name__ == "__main__":
    ...
    problem_pddls = []
    for i in range(3, 11):
        problem_pddls += sorted([str(i)+'/'+ p for p in os.listdir("../benchmarks/blocksworld/"+str(i))])
    ...
    problems = args.get_strips_problems()
    for problem in problems:
        state_value_pairs, _ = get_optimal_actions_using_py(problem, mode={'mode':'eval', 'search':'novelty'})

        if state_value_pairs:
            print(problem.name)
            optimal_pairs, _ = get_optimal_actions_using_py(problem, mode={'mode':'eval', 'search':'lm-cut'})
        
            if len(state_value_pairs)!= len(optimal_pairs):
                print(problem.problem_pddl, len(state_value_pairs), len(optimal_pairs))
                break